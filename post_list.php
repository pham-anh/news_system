                    ...
                    if (!isset($_GET['post_id'])) {
                        foreach ($posts as $post) {
                            ...
                            if ($time_ago < 60 * 60) {
                                ...
                                $time_display = date('H:i Y-m-d', time($post['updated_at']));
                            }
                            ?>
                            <div>

                                <h2><?php echo $post['subject'] ?></h2>
                                <p title="<?php echo date('D j M Y H:i', $post['updated_timestamp']) ?>"><span class="glyphicon glyphicon-time" aria-hidden="true"></span><span class="label label-success"> <?php echo $time_display ?></span></p>
                        ...
                        foreach ($posts as $post) {
                            if ($post['id'] != $post_id) {
                                ...
                                <!--<div class="">-->
                                <h2><?php echo $post['subject'] ?></h2>
                                <p><span class="glyphicon glyphicon-time" aria-hidden="true"></span> <?php echo date('D j M Y H:i', $post['updated_timestamp']) ?> &nbsp;&nbsp;&nbsp;&nbsp;<button class="small"><a href="post_edit.php?post_id=<?php echo $post_id ?>">Edit post</a></button></p>
                                <?php if ($post['img'] != '') {
                                    ?>
